...
\end{lineq}
is in a conflict with the initial condition (since $\vec{n} = \left(\begin{smallmatrix} 0 \\ 1 \end{smallmatrix} \right)$), which is again physically incorrect, and produces further computational costs. The reason for the extra computational costs is that the nonlinear equation solver iterates on the bottom boundary, which is in an ideal case out of the reach of the infiltration experiment, and naturaly there is no physical process taking place in the initial time levels. The computational process, that is produced on the bottom boundary in the initial time levels with such boundary setup, originates from the initial and boundary condition mismatch. 
...
The left hand side domain boundary should be located at a sufficient distance from the axis of anisotropy, because of the term $\frac{1}{r}$ in \eqref{convect}. The Peclet number representing the numerical stability of convection-diffusion problems (see e.g. \citep{Knobloch2008}) is in our case given as follows
\begin{lineq}
