...
\end{lineq}
Since the flow pattern during our SR experiment is analogous to the flow pattern originating from infiltration well (in a horizontaly homogeneous medium it is rotationally symmetric flow), we could easily reduce the spatial dimension by employing cylindrical coordinates -- $z$ for the horizontal dimension and $r$ for the radial dimension (angular dimension is not employed here due rotationally symmetric flow pattern). It is a standardized approach used for well-flow systems, see eg. \citep{harr-1962}. 

For rotationally symmetric flow, the Richards equation can be formulated in cylindrical cooardinates as
\begin{lineq}
\label{richaxi}
...
Physically correct boundary conditions for the bottom boundary is either the Neumann no-flow boundary at the impermeable layer or Dirichlet boundary both at the groundwater table. We chose a constant Dirichlet boundary condition. {The minimum depth} of the groundwater table was estimated at -280~cm below the surface, {see the explanation given in section~\ref{site},} and  we assume that the water table remains constant during the experiment. With the duration of the SR infiltration experiment, and the volume of water added to the soil, the groundwater table is not affected. With this particular setup the domain became extremely narrow and deep, see figure~\ref{valec}. \linelabel{line:dbc}. 
...
The Richards equation formulated for cylindrical coordinates~\eqref{richaxi} falls into the cathegory of convection-diffusion equations, and so the well known stability restrictions for numerical solutions of these problems appear~\citep{zienkiewicz1976}.
The Peclet number representing the numerical stability of convection-diffusion problems is defined as~\citep{knobloch2008} 
\begin{lineq}
\label{peclet-std}
Pe = \frac{||\vec{v}|| \Delta x}{2 D},
\end{lineq}
where $\vec{v}$ is the convection coefficient, $\Delta x$ is the discretization step, and $D$ is the diffusion (for isotropic setup). Based on the definitions given above, the convection term $v_r$ (with a respect to $r$ axis) and the diffusion term $D$ is considered here as
\begin{lineq}
v_r = -\frac{1}{r}K(h), \quad D = K(h),
\end{lineq}
and so the equation~\eqref{peclet-std} can be further formulated as
\begin{lineq}
\label{peclet-re}
Pe =  \frac{\frac{1}{r}K(h) \Delta x}{2K(h)} = \frac{\Delta x}{2r}.
\end{lineq}
Since our mesh is triangular, $\Delta x$ can be roughly assumed to be the greatest triangle altitude (since we assume some mesh quality properties). Then a sufficient distance from the axis of axisymmetry is such that the Peclet number is sufficiently low. It also turns out that we should avoid domain setups where $r=0$. If we want to make our computation free of the well
