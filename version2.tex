...
\subsection{Stability restrictions of convection dominant problems}
...
\begin{lineq}
\label{peclet-std}
Pe = \frac{||\vec{q}|| \Delta x}{2 D},
\end{lineq}
where $||\vec{q}||$ is the convection vector length, $\Delta x$ is the discretization step, and $D$ is the diffusion (for isotropic setup). Based on the definitions given above the equation~\eqref{peclet-std} can be formulated as
\begin{lineq}
